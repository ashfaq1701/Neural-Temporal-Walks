...
class NeurTWs(torch.nn.Module):
    ...
    def contrast(self, src_idx_l, tgt_idx_l, bgd_idx_l, cut_time_l, e_idx_l=None):
        ...
        # ---- Forward ----
        src_embed, t_src = self.forward_msg(src_idx_l, cut_time_l, subgraph_src)
        tgt_embed, t_tgt = self.forward_msg(tgt_idx_l, cut_time_l, subgraph_tgt)

        timing['position'] += t_src['position'] + t_tgt['position']
        timing['model'] += t_src['model'] + t_tgt['model']
        ...
        for i in range(self.negatives):
            ...
            # Forward
            bgd_embed, t_bgd = self.forward_msg(bgd_idx_cut, cut_time_l, subgraph_bgd)
            timing['position'] += t_bgd['position']
            timing['model'] += t_bgd['model']
    ...
    def inference(self, src_idx_l, tgt_idx_l, bgd_idx_l, cut_time_l, e_idx_l=None):
        ...
        src_embed, _ = self.forward_msg(src_idx_l, cut_time_l, subgraph_src)
        tgt_embed, _ = self.forward_msg(tgt_idx_l, cut_time_l, subgraph_tgt)
        ...
        bgd_embed, _ = self.forward_msg(bgd_idx_l, cut_time_l, subgraph_bgd)
    ...
    def forward_msg(self, src_idx_l, cut_time_l, subgraph_src):
        ...
        final_node_embeddings = self.forward_msg_walk(
            hidden_embeddings, edge_features, position_features, t_records_th, None
        )
        timing['model'] += perf_counter() - t0
